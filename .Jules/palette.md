...
## 2026-04-18 - [Bilingual State-Aware Accessibility]
**Learning:** In bilingual interfaces, ARIA labels must not only translate but also reflect the dynamic state of interactive elements (e.g., toggling between "Open Menu" and "Close Menu" or "Play" and "Pause"). This ensures screen reader users have the same immediate feedback as sighted users across both supported languages.
**Action:** Implemented state-aware bilingual ARIA labels for the global navigation menu using vanilla JS attribute manipulation, and added high-visibility focus states in CSS.
