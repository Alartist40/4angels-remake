...
## 2026-05-01 - [Navigation Active State Pattern]
**Learning:** For multi-page navigation, using `aria-current="page"` is the gold standard for accessibility. It provides clear semantic feedback to screen readers that a visual "active" class cannot. Combining it with visual cues like bold text and a decorative underline ensures a universally intuitive experience.
**Action:** Replaced the non-semantic `.active` class with `aria-current="page"` on all navigation links and added corresponding CSS styles for visual feedback.
