...
## 2026-04-19 - Semantic Navigation and Escape Support
**Learning:** Providing semantic state information (`aria-current="page"`) is critical for screen reader users to understand their location in the site hierarchy. Additionally, interactive overlays like mobile menus should always support the 'Escape' key for closure to match user mental models for "dismissing" temporary UI states.
**Action:** Implemented automatic `aria-current` assignment based on the current URL and added an Escape key listener to the mobile navigation toggle that restores focus after closure.
